...
class CommentSerializer(serializers.ModelSerializer):
    author_2 = SlugRelatedField(slug_field='first_name',read_only=True)
    # author = SlugRelatedField(slug_field='feedback_text',read_only=True)

    def create(self, validated_data):
        if self.context.get('author_2_id',None):
            validated_data['author_2_id']=self.context.get('author_2_id')
        else:
            validated_data['author_2_id']=self.context['request'].user.pk
